...
class KeywordsDict:
    ...
    from .Expressions.branchingAndCycles import (
        IfExpression,
        EndIfExpression,
        WhileExpression,
        EndWhileExpression,
    ...
    from .Expressions.function import (
        ...
        EndFunctionDefinitionExpression,
        ReturnExpression,
    )
    ...
    def GetExpression(self, command):
        try:
            ...
            raise MHscr_KeywordError(
                f"Unknown keyword {err}"
                if command not in self.runner.variables.keys()
                else f"Cannot change value of an initialized constant {command}",
                command=command,
            )
