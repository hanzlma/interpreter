...
class Int:
    ...
    def __init__(self, val: str) -> None:
        self.value = int(
            val
        )  # error handling in managing functions (if this function throws, the value is not an integer)
    ...
    def __init__(self, val: str | bool, _dynamically_called: bool = False) -> None:
        if isinstance(val, bool):
            ...
            raise MHscr_ValueError(
                f"Dynamically typed value '{val}' can only be turned to Bool if it is True or False"
            )
...
class Let:
    ...
    def __init__(self, val: str | String | Bool | Float | Int) -> None:
        self.value = (
            val.value
            if isinstance(val, (String, Bool, Float, Int, Let))
            else GetDatatypeDynamically(f"'{val}'")
        )
