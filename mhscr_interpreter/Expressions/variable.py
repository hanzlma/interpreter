...
from mhscr_interpreter.errors import MHscr_KeywordError, MHscr_OperatorError, MHscr_TypeError, MHscr_ValueError, MHscr_SyntaxError, getErrorLine
...
class VariableExp(Expression):
    ...
    def prepareArguments(self)->None:
        try:
            ...
            if parts[2] != '=':
                raise MHscr_OperatorError(f"Unexpected value {parts[2]}", line=getErrorLine(self.runner, self) if not self.cli else None)
            self.argument = self.inp.replace(f"{parts[0]} {parts[1]} {parts[2]} ", "")
            self.arguments = SplitByOperators(self.argument)
        except IndexError:
            raise MHscr_SyntaxError("Wrong arguments for Variable Expression", line=getErrorLine(self.runner, self) if not self.cli else None)
    ...
    def execute(self, /, *, functionCall=False) -> None:
        ...
        if self.name in self.runner.variables:
            raise MHscr_ValueError(f"Variable '{self.name}' already initialized.", line=getErrorLine(self.runner, self) if not self.cli else None)
...
class ConstantVariableExp(Expression):
    ...
    def execute(self, /, *, functionCall=False) -> None:
        super().execute(functionCall=functionCall)
        VariableExp(self.runner,self.inp, self.cli, True).execute(functionCall=functionCall)
...
class VariableAssignmentExp(Expression):
    ...
    def execute(self, /, *, functionCall=False) -> None:
        ...
        if self.name not in self.runner.variables:
            raise MHscr_ValueError(f"Variable {self.name} not initialized", line=getErrorLine(self.runner, self) if not self.cli else None)
        ...
        if self.runner.variables[self.name].const:
            raise MHscr_TypeError("Cannot assign value to constant.",line=getErrorLine(self.runner, self) if not self.cli else None)
    ...
    def prepareArguments(self) -> None:
        ...
        self.name = parts[0]
        if parts[1] != '=':
            raise MHscr_OperatorError(f"Unexpected operator \"{parts[1]}\"", line=getErrorLine(self.runner, self) if not self.cli else None)
