...
class FunctionDefinitionExpression(Expression):
    ...
    def prepareArguments(self) -> None:
        ...
        self.arguments = [
            arg.strip() for arg in self.inp.replace(f"fn {self.name}", "").split(",")
        ]
        ...
        for argument in self.arguments:
            if argument == "":
                continue
            datatype = VariableExp.GetDatatype(
                self.runner, self, argument.split(" ")[0]
            )
    ...
    def getExpressions(self) -> None:
        ...
        if not any(
            [
                isinstance(expression, EndFunctionDefinitionExpression)
    ...
    def execute(self, /, *, functionCall=False) -> None:
        ...
        if functionCall:
            raise MHscr_RuntimeError(
                "Cannot define a function inside a function",
                line=self.runner.source_expressions.index(self),
            )
        self.getExpressions()

        if any([fn.name == self.name for fn in self.runner.functions]):
            raise MHscr_RuntimeError(
                f"Function '{self.name}' is already defined.",
                line=self.runner.source_expressions.index(self),
            )

        self.runner.functions.append(
            Function(self.name, self.defined_arguments, self.expressions)
        )
...
class FunctionCallExpression(Expression):
    ...
    def __init__(self, runner, inp: str, cli: bool) -> None:
        ...
        self.arguments = [
            arg.strip() for arg in self.inp.replace(self.name, "").split(",")
        ]
    ...
    def execute(self, /, *, functionCall=False) -> None:
        ...
        for i in range(len(self.function.arguments)):
            ...
            if name in self.runner.variables.keys():
                raise MHscr_RuntimeError(
                    f"Variable {name} already initialized.",
                    line=self.runner.source_expressions.index(self),
                )
            self.runner.variables[name] = Variable(
                name,
                datatype,
                GetDatatypeDynamically(self.runner, self.arguments[i]),
                local=True,
            )
        ...
        for expression in self.function.expressions:
            expression.execute(
                functionCall=next(
                    f for f in self.runner.functions if f == self.function
                )
            )
...
class ReturnExpression(Expression):
    ...
    def __init__(self, runner, inp, cli):
        super().__init__(runner, inp, cli)
        self.arguments = (
            SplitByOperators(self.inp.replace("return ", ""))
            if self.inp.replace("return", "") != ""
            else None
        )
    ...
    def execute(self, /, *, functionCall=False):
        ...
        if functionCall is False:
            raise MHscr_SyntaxError(
                "Return cannot be located outside of a function block."
            )

        functionCall.returnValue = (
            PrepareValue(self.runner, self.inp.replace("return ", ""), self.arguments)
