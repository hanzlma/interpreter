...
class DynamicCalculator:
    ...
    def CalculateDynamicOperations(
        arguments: list[String | Int | Float | Bool | Let],
        ops: list[str],
        ...
        for i in range(len(arguments)):
            arguments[i] = (
                GetDatatypeDynamically(str(arguments[i]))
                if isinstance(arguments[i], Let)
                else arguments[i]
            )
        if isinstance(arguments[0], String) and (
            ops.count(operators["minus"]) > 0
            or ops.count(operators["multiply"]) > 0
            or ops.count(operators["divide"]) > 0
        ):
            raise MHscr_TypeError("Strings can only be summed")
        if (
            not isinstance(arguments[0], (String, Let))
        ) and DynamicListContainsDatatype(arguments, String):
            raise MHscr_TypeError(
                "String can only be used with other datatypes if it is the first type in the operation"
            )
        value: String | Int | Float | Bool
        (arguments, ops) = DynamicCalculator.ResolveNumericalOperatorPrecedences(
            arguments, ops
        )
        if not _recursive:
            (arguments, ops) = DynamicCalculator.ResolveLogicalOperatorPrecedences(
                arguments, ops
            )
            (arguments, ops) = DynamicCalculator.ResolveComparativeOperatorPrecedences(
                arguments, ops
            )
    ...
    def ResolveNumericalOperatorPrecedences(
        arguments: list[String | Int | Float | Bool], ops: list[str]
    ) -> tuple[list[String | Int | Float | Bool], list[str]]:
    ...
    def ResolveLogicalOperatorPrecedences(
        arguments: list[String | Int | Float | Bool], ops: list[str]
    ) -> tuple[list[String | Int | Float | Bool], list[str]]:
        ...
        for i in range(len(logicalOperators)):
            if i == 0:
                outputArguments.append(
                    DynamicCalculator.CalculateDynamicOperations(
                        arguments[: logicalOperators[i][0] + 1],
                        ops[: logicalOperators[i][0]],
                        True,
                    )
                )
            else:
                outputArguments.append(
                    DynamicCalculator.CalculateDynamicOperations(
                        arguments[
                            logicalOperators[i - 1][0] + 1 : logicalOperators[i][0] + 1
                        ],
                        ops[logicalOperators[i - 1][0] + 1 : logicalOperators[i][0]],
                        True,
                    )
                )
    ...
    def ResolveComparativeOperatorPrecedences(
        arguments: list[String | Int | Float | Bool], ops: list[str]
    ) -> tuple[list[String | Int | Float | Bool], list[str]]:
        if len(re.findall(r"==|!=|>|<|>=|<=", str(ops))) < 1:
            return (arguments, ops)
    ...
    def Operation(
        left: String | Int | Float | Bool,
        right: String | Int | Float | Bool,
        operator: str,
    ) -> String | Int | Float | Bool:
        operations: dict[str,] = {
    ...
    def Sum(
        left: String | Int | Float | Bool, right: String | Int | Float | Bool
    ) -> String | Int | Float | Bool:
        T: type = type(left)
        ...
        elif (T is Int and type_argument is Float) or (
            T is Float and type_argument is Int
        ):
            return Float(left.value + right.value)
        else:
            raise MHscr_ValueError(
                f"Unsupported operation: {left} {T} + {right} {type_argument}"
            )
    ...
    def Substraction(
        ...
        if T is Bool or T is String:
            raise MHscr_TypeError(
                f"Type {T} cannot be used in a substracting operation."
            )
        elif type_argument is Bool or type_argument is String:
            raise MHscr_TypeError(
                f"Type {type_argument} cannot be used in a substracting operation."
            )
    ...
    def Multiplication(
        ...
        if T is Bool or T is String:
            raise MHscr_TypeError(
                f"Type {T} cannot be used in a multiplicating operation."
            )
        elif type_argument is Bool or type_argument is String:
            raise MHscr_TypeError(
                f"Type {type_argument} cannot be used in a multiplicating operation."
            )
        elif T is Float or type_argument is Float:
            return Float(left.value * right.value)
        elif T is Int and T is type_argument:
    ...
    def Division(
        ...
        try:
            if T is Bool or T is String:
                raise MHscr_TypeError(
                    f"Type {T} cannot be used in a divising operation."
                )
            elif type_argument is Bool or type_argument is String:
                raise MHscr_TypeError(
                    f"Type {type_argument} cannot be used in a divising operation."
                )
            elif T is Float or type_argument is Float:
                return Float(left.value / right.value)
            elif T is Int and T is type_argument:
    ...
    def LogicalAnd(
        ...
        try:
            ...
            raise MHscr_ValueError(
                f"Unsupported operation: {left} {type(left)} && {right} {type(right)}"
            )
    ...
    def PrintAllArgumentsAndOperators(
        arguments: list[String | Int | Float | Bool], ops: list[str]
    ) -> None:
        a = []
