...
class WholefileRunner:
    ...
    def Run(self) -> None:
        ...
        try:
            ...
            print(
                f"Program encountered an exception: line {lines.index(err.command)}, {err.get_name()}: {err.message}"
            )
            return

        try:
            self.expressions = [
                commands[i](runner=self, inp=lines[i], cli=False)
                for i in range(len(commands))
            ]

        except MHscr_Error as err:
            print(
                f"Program encountered an exception: line {err.line if err.line else lines.index(err.command)}, {err.get_name()}: {err.message}"
            )
            return
        ...
        for expression in self.expressions:
            try:
                ...
                print(
                    f"Program encountered an exception: line {err.line if err.line else self.expressions.index(expression)}, {err.get_name()}: {err.message}"
                )
                return
